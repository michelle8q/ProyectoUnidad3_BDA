...
/**
 * Clase DAO que se encarga de manejar las operaciones de persistencia
 * relacionadas con las canciones.
 *
 * En esta base de datos las canciones no se guardan como una coleccion aparte,
 * sino dentro de la lista de canciones de cada album, por eso esta clase
 * trabaja sobre la coleccion albumes cuando necesita guardar o consultar
 * canciones.
 *
 * @author cinca luisf
 *
...
public class CancionDAO implements ICancionDAO {
     ...
     *
     * @param conexion objeto encargado de proporcionar la conexion a la base de
     * datos.
     */
    ...
    /**
     * Guarda una nueva cancion dentro de un album existente. Como las canciones
     * no tienen una coleccion propia en la base de datos se agregan
     * directamente dentro del arreglo "canciones" del album. Por lo tanto se
     * recibe el id del album donde se quiere guardar la cancion.
     *
     ...
     * @throws PersistenciaException si no se encuentra el album o si ocurre un
     * error con MongoDB.
     */
    @Override
    ...
    /**
     * Busca canciones por texto libre, para el buscador general de la
     * aplicacion.
     *
     ...
     * @return lista de canciones que coinciden con el texto, con su informacion
     * de contexto.
     * @throws PersistenciaException si ocurre un error al consultar la base de
     * datos.
     */
    ...
    @Override
    public CancionDetallesDTO obtenerPorId(String id) throws Exception {
        try {
            MongoDatabase bd = conexion.conexion();
            MongoCollection<Document> collection = bd.getCollection("albumes");
            ...
            Bson etapaFiltro = Aggregates.match(Filters.eq("canciones._id", new ObjectId(id)));
            ...
            Document doc = collection.aggregate(Arrays.asList(etapaUnwind, etapaFiltro, etapaProyeccion)).first();

            if (doc != null) {

                CancionDetallesDTO cancion = new CancionDetallesDTO();
                cancion.setId(doc.getObjectId("_id").toString());
                cancion.setNombre(doc.getString("nombre"));

                if (doc.containsKey("duracion")) {
                    cancion.setDuracion(doc.getString("duracion"));
                }
                if (doc.containsKey("nombreAlbum")) {
                    cancion.setNombreAlbum(doc.getString("nombreAlbum"));
                }
                if (doc.containsKey("imagenAlbum")) {
                    cancion.setImagenAlbum(doc.getString("imagenAlbum"));
                }
                if (doc.containsKey("nombreArtista")) {
                    cancion.setNombreArtista(doc.getString("nombreArtista"));
                }

                if (doc.get("genero") != null) {
                    Document generoDoc = (Document) doc.get("genero");
                    if (generoDoc.containsKey("nombre")) {
                        cancion.setGenero(generoDoc.getString("nombre"));
                    }
                }
            ...
            throw new Exception("Error al buscar la canción por ID en la BD: " + ex.getMessage());
        }
    }

    @Override
    public CancionDetallesDTO obtenerPorNombre(String nombre) throws Exception {
        try {
            MongoDatabase bd = conexion.conexion();
            MongoCollection<Document> collection = bd.getCollection("albumes");
            ...
            Bson etapaFiltro = Aggregates.match(Filters.eq("canciones.nombre", nombre));
            ...
            throw new Exception("Error al buscar la canción por nombre en la BD: " + ex.getMessage());
        }
    }
